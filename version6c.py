...
#break ends the check when the right credentials are entered
    ...
    def open_workout(self):
        # Refresh the workout exercise dropdown with the latest known exercises
    ...
    # Reads CSV and returns sorted list of unique exercises this user has logged before
    ...
    # Opens analytics and pre-populates the scrollable exercise picker with known exercises
    def open_analytics(self):
        self.refresh_analytics_exercise_list()
        self.show_frame(self.analytics_frame)

    # Rebuilds the listbox inside the analytics exercise picker popup data
    def refresh_analytics_exercise_list(self):
        self._analytics_exercises = self.get_known_exercises()
...
#new workout tab
    def setup_workout_frame(self):
        ...
        # Dropdown for previously entered exercises (workout screen)
    ...
    # One set shown to start; Add Set button adds more columns dynamically
    def add_exercise_row(self):
        ...
        # Add Set button for this exercise row
...
#Uses .winfo_children().destroy() to delete the dynamically generated widgets from the screen after saving
    ...
    def open_history(self):
        ...
        if os.path.exists(WORKOUT_FILE):
            with open(WORKOUT_FILE, mode='r') as file:
                ...
                # Filter rows for this user then reverse so newest entries appear at the top
    ...
    # Analytics - 3 tabs: 1RM Progress, Volume Per Session, Top Exercises
    def setup_analytics_frame(self):
        ...
        # --- ADDED: Scrollable exercise picker for analytics ---
        # Instead of a plain OptionMenu (which doesn't scroll), this uses a button that opens
        # a small popup window containing a Listbox with a Scrollbar. This handles long exercise
        # lists cleanly without the dropdown flying off the screen.
        ...
        # The entry box still exists so the user can also type manually if they want
        self.ana_entry = tk.Entry(ctrl_frame, font=FONT_BODY, width=20)
        self.ana_entry.pack(side=tk.LEFT, padx=(10, 4))

        # Button that opens the scrollable picker popup
        ...
        self._analytics_exercises = []

    # --- ADDED: Opens a small popup window with a scrollable Listbox of exercises ---
    # This solves the tkinter OptionMenu scroll problem. The popup is positioned near the
    # Pick button, shows all known exercises in a Listbox with a scrollbar attached,
    # and clicking an item fills the entry box and closes the popup automatically.
    def open_exercise_picker(self):
        ...
        # Create a floating Toplevel window (no title bar chrome, just the list)
        popup = tk.Toplevel(self)
        popup.title("")
        popup.resizable(False, False)
        popup.grab_set()  # Makes the popup modal - blocks interaction with the main window until closed

        # Position the popup roughly below the analytics control row
        ...
        # Populate the listbox with all known exercises for this user
        for ex in exercises:
            listbox.insert(tk.END, ex)

        # Clicking an item fills the entry and closes the popup
        ...
        listbox.bind("<Double-Button-1>", on_select)  # Double-click to select
        listbox.bind("<Return>", on_select)            # Enter key to select

        tk.Button(popup, text="Select", bg=BTN_COLOR, fg="white", font=("Arial", 10, "bold"),
                  command=on_select).pack(pady=(0, 5))

    # Switches which tab is active and auto-generates Top Exercises (no search needed for that one)
    ...
    def generate_top_exercises(self):
        ...
        for i, (ex, count) in enumerate(top):
            ...
            # --- MODIFIED: Increased exercise name character limit from 10 to 20 ---
